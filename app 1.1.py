...
def delete_owner(id):
    try:
        ...
        # Query para verificar a existência do proprietário ativo.
        sql = "SELECT owner_id FROM owner WHERE owner_id = ? AND owner_status != 'off'"

        # Executa a query.
        cursor.execute(sql, (id,))

        # Obtém o resultado da consulta para 'owner_row'.
        owner_row = cursor.fetchone()

        # Se o proprietário existe e está ativo...
        if owner_row:
            # Query para atualizar o status do proprietário para 'off'.
            ...
            # Retorna mensagem de sucesso e status HTTP "200 Ok".
            return {"success": "Proprietário desativado com sucesso", "id": id}, 200

        # Se o proprietário não existe ou já está inativo.
        else:
            # Fecha a conexão com o banco de dados.
            conn.close()
